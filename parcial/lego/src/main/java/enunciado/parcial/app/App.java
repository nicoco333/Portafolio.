...
public class App {
    public static void main(String[] args) {
        ...
        try {
            ...
            // --- Smoke test: crear EntityManager y contar registros en COUNTRIES ---
            try {
                var em = enunciado.parcial.repositories.context.LegoDbContext.getInstance().getManager();
                Long countries = em.createQuery("SELECT COUNT(c) FROM Country c", Long.class).getSingleResult();
                System.out.println("[OK] DB init + JPA mappings verificados. COUNTRIES count=" + countries);
            } catch (Throwable t) {
                System.err.println("[FAIL] DB init o JPA mapping falló: " + t.getMessage());
                t.printStackTrace();
                System.exit(1);
            }
        } catch (Exception e) {
